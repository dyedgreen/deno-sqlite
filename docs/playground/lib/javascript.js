  ...
  CodeMirror.defineMode("javascript", function (config, parserConfig) {
    ...
    function tokenBase(stream, state) {
      ...
      if (ch == '"' || ch == "'") {
        ...
        ch == "<" && stream.match("!--") || ch == "-" && stream.match("->")
      ) {
        stream.skipToEnd();
        return ret("comment", "comment");
        ...
        if (state.lastType != ".") {
          ...
          if (
            word == "async" && stream.match(/^(\s|\/\*.*?\*\/)*[\[\(\w]/, false)
          ) {
    ...
    function maybeoperatorNoComma(type, value, noComma) {
      ...
      if (type == "operator") {
        ...
        if (
          isTS && value == "<" && cx.stream.match(/^([^>]|<.*?>)*>\s*\(/, false)
        ) {
    ...
    function typearg(type, value) {
      if (
        type == "variable" && cx.stream.match(/^\s*[?:]/, false) || value == "?"
      ) {
        return cont(typearg);
      }
      if (type == ":") return cont(typeexpr);
