  ...
  (function () {
    ...
    var bidiOrdering = (function () {
      ...
      return function (str, direction) {
        ...
        if (
          str.length == 0 || direction == "ltr" && !bidiRE.test(str)
        ) {
          return false;
        }
        ...
        for (var i$3 = 1, prev$1 = types[0]; i$3 < len - 1; ++i$3) {
          ...
          if (
            type$2 == "+" && prev$1 == "1" && types[i$3 + 1] == "1"
          ) {
            types[i$3] = "1";
          } else if (
            type$2 == "," && prev$1 == types[i$3 + 1] &&
            (prev$1 == "1" || prev$1 == "n")
          ) {
            types[i$3] = prev$1;
          }
    ...
    function zeroWidthElement(measure) {
      ...
      var node = zwspSupported ? elt("span", "\u200b") : elt(
        "span",
        "\u00a0",
        null,
    ...
    function getContextBefore(cm, n, precise) {
      ...
      doc.iter(start, n, function (line) {
        ...
        line.stateAfter =
          pos == n - 1 || pos % 5 == 0 || pos >= display.viewFrom &&
          pos < display.viewTo
            ? context.save()
            : null;
        context.nextLine();
    ...
    function findStartLine(cm, n, precise) {
      ...
      for (var search = n; search > lim; --search) {
        ...
        if (
          after && (!precise || search + (after instanceof SavedContext
            ? after.lookAhead
            : 0) <= doc.modeFrontier)
        ) {
          return search;
      ...
      if (old) {
        for (var i = 0; i < old.length; ++i) {
          ...
          var startsBefore = span.from == null || (marker.inclusiveLeft
            ...
            : span.from < startCh);
          if (
            startsBefore || span.from == startCh && marker.type == "bookmark" &&
      ...
      if (old) {
        for (var i = 0; i < old.length; ++i) {
          ...
          var endsAfter = span.to == null || (marker.inclusiveRight
            ...
            : span.to > endCh);
          if (
            endsAfter || span.from == endCh && marker.type == "bookmark" &&
            (!isInsert || span.marker.insertLeft)
          ) {
            var startsBefore = span.from == null || (marker.inclusiveLeft
              ? span.from <= endCh
    ...
    function collapsedSpanAround(line, ch) {
      ...
      if (sps) {
        for (var i = 0; i < sps.length; ++i) {
          var sp = sps[i];
          if (
            sp.marker.collapsed && (sp.from == null || sp.from < ch) &&
            (sp.to == null || sp.to > ch) &&
            (!found || compareCollapsedMarkers(found, sp.marker) < 0)
          ) {
            found = sp.marker;
      ...
      if (style || startStyle || endStyle || mustWrap || css) {
        ...
        if (attributes) {
          for (var attr in attributes) {
            if (
              attributes.hasOwnProperty(attr) && attr != "style" &&
              attr != "class"
            ) {
        ...
        if (nextChange == pos) { // Update current marker set
          ...
          for (var j = 0; j < spans.length; ++j) {
            ...
            if (m.type == "bookmark" && sp.from == pos && m.widgetNode) {
              ...
              sp.from <= pos &&
              (sp.to == null || sp.to > pos || m.collapsed && sp.to == pos &&
                sp.from == pos)
            ) {
    ...
    function ensureLineHeights(cm, lineView, rect) {
      ...
      if (
        !lineView.measure.heights || wrapping &&
      ...
      for (var i = 0; i < map$$1.length; i += 3) {
        ...
        if (start != null) {
          ...
          if (bias == "left" && start == 0) {
            while (
              i && map$$1[i - 2] == map$$1[i - 3] && map$$1[i - 1].insertLeft
            ) {
              node = map$$1[(i -= 3) + 2];
              collapse = "left";
            }
          ...
          if (bias == "right" && start == mEnd - mStart) {
            while (
              i < map$$1.length - 3 && map$$1[i + 3] == map$$1[i + 4] &&
    ...
    function measureCharInner(cm, prepared, ch, bias) {
      ...
      if (
        ie && ie_version < 9 && !start && (!rect || !rect.left && !rect.right)
      ) {
    ...
    function posFromMouse(cm, e, liberal, forRect) {
      ...
      if (
        !liberal && e_target(e).getAttribute("cm-not-content") == "true"
      ) {
        return null;
      }
    ...
    function adjustView(cm, from, to) {
      ...
      if (
        view.length == 0 || from >= display.viewTo || to <= display.viewFrom
      ) {
      ...
      if (doScroll != null && !phantom) {
        var scrollNode = elt(
          ...
          ("position: absolute;\n                         top: " +
            ...
            "px;\n                         left: " + (rect.left) +
            "px; width: " + (Math.max(2, rect.right - rect.left)) + "px;"),
        );
    ...
    function setScrollLeft(cm, val, isScroller, forceScroll) {
      ...
      if (
        (isScroller
          ...
          : Math.abs(cm.doc.scrollLeft - val) < 2) && !forceScroll
      ) {
        return;
      }
    ...
    function endOperation_R1(op) {
      ...
      op.mustUpdate = op.viewChanged || op.forceUpdate ||
        op.scrollTop != null ||
    ...
    function restoreSelection(snapshot) {
      if (
        !snapshot || !snapshot.activeElt || snapshot.activeElt == activeElt()
      ) {
        return;
      }
      snapshot.activeElt.focus();
      if (
        snapshot.anchorNode && contains(document.body, snapshot.anchorNode) &&
    ...
    function lastChangeEvent(hist, force) {
      if (force) {
        ...
        hist.done.length > 1 && !hist.done[hist.done.length - 2].ranges
      ) {
        hist.done.pop();
        return lst(hist.done);
      }
    ...
    function addChangeToHistory(doc, change, selAfter, opId) {
      ...
      if (
        ...
        if (
          cmp(change.from, change.to) == 0 && cmp(change.from, last.to) == 0
        ) {
    ...
    function setSelectionNoUndo(doc, sel, options) {
      if (
        hasHandler(doc, "beforeSelectionChange") || doc.cm &&
    ...
    function skipAtomicInner(doc, pos, oldPos, dir, mayClear) {
      ...
      if (line.markedSpans) {
        for (var i = 0; i < line.markedSpans.length; ++i) {
          ...
          var preventCursorLeft = ("selectLeft" in m)
            ? !m.selectLeft
            : m.inclusiveLeft;
          ...
          if (
            (sp.from == null || (preventCursorLeft
              ? sp.from <= pos.ch
              : sp.from < pos.ch)) &&
            (sp.to == null || (preventCursorRight
              ? sp.to >= pos.ch
              : sp.to > pos.ch))
          ) {
            if (mayClear) {
            ...
            if (oldPos) {
              ...
              if (
                near && near.line == pos.line && (diff = cmp(near, oldPos)) &&
                (dir < 0 ? diff < 0 : diff > 0)
              ) {
    ...
    function movePos(doc, pos, dir, line) {
      if (dir < 0 && pos.ch == 0) {
        ...
        dir > 0 && pos.ch == (line || getLine(doc, pos.line)).text.length
      ) {
    ...
    function makeChange(doc, change, ignoreReadOnly) {
      ...
      if (
        hasHandler(doc, "beforeChange") || doc.cm &&
    ...
    TextMarker.prototype.clear = function () {
      ...
      for (var i = 0; i < this.lines.length; ++i) {
        ...
        if (
          span.from == null && this$1.collapsed &&
    ...
    TextMarker.prototype.attachLine = function (line) {
      if (!this.lines.length && this.doc.cm) {
        ...
        if (
          !op.maybeHiddenMarkers || indexOf(op.maybeHiddenMarkers, this) == -1
        ) {
    ...
    function markText(doc, from, to, options, type) {
      ...
      if (cm) {
        ...
        if (marker.collapsed) {
          ...
          marker.className || marker.startStyle || marker.endStyle ||
          marker.css ||
        ...
        this.iter(from.line, to.line + 1, function (line) {
          ...
          if (spans) {
            for (var i = 0; i < spans.length; i++) {
              var span = spans[i];
              if (
                !(span.to != null && lineNo$$1 == from.line &&
                  from.ch >= span.to ||
    ...
    function moveVisually(cm, line, start, dir) {
      ...
      var partPos = getBidiPartAt(bidi, start.ch, start.sticky),
        part = bidi[partPos];
      if (
        cm.doc.direction == "ltr" && part.level % 2 == 0 &&
    ...
    function onKeyDown(e) {
      ...
      if (presto) {
        ...
        if (
          !handled && code == 88 && !hasCopyEvent && (mac
            ? e.metaKey
            : e.ctrlKey)
        ) {
      ...
      if (
        gecko && !mac && !handled && code == 46 && e.shiftKey && !e.ctrlKey &&
    ...
    function onKeyPress(e) {
      var cm = this;
      if (
        eventInWidget(cm.display, e) || signalDOMEvent(cm, e) || e.ctrlKey &&
      ...
      if (
        (presto && (!e.which || e.which < 10)) && handleKeyBinding(cm, e)
      ) {
        return;
      }
    ...
    function onMouseDown(e) {
      ...
      if (
        signalDOMEvent(cm, e) || display.activeTouch &&
    ...
    function leftButtonSelect(cm, event, start, behavior) {
      ...
      if (!behavior.addNew) {
        ...
        ranges.length > 1 && ranges[ourIndex].empty() &&
      ...
      function extend(e) {
        ...
        if (cmp(cur, lastPos) != 0) {
          ...
          var outside = e.clientY < editorSize.top
            ? -20
    ...
    function bidiSimplify(cm, range$$1) {
      ...
      if (head.line != anchor.line) {
        ...
        var dir = headIndex - index || (head.ch - anchor.ch) * (part.level == 1
          ? -1
          : 1);
    ...
    function gutterEvent(cm, e, type, prevent) {
      ...
      if (
        mY > lineBox.bottom || !hasHandler(cm, type)
      ) {
    ...
    function registerEventHandlers(cm) {
      ...
      if (ie && ie_version < 11) {
        on(
          ...
          operation(cm, function (e) {
            ...
            var pos = posFromMouse(cm, e);
            if (
              !pos || clickInGutter(cm, e) || eventInWidget(cm.display, e)
            ) {
              return;
            }
            e_preventDefault(e);
      ...
      on(d.scroller, "touchend", function (e) {
        ...
        if (
          touch && !eventInWidget(d, e) && touch.left != null &&
      ...
      for (var i$1 = sel.ranges.length - 1; i$1 >= 0; i$1--) {
        ...
        if (range$$1.empty()) {
          if (deleted && deleted > 0) { // Handle deletion
            ...
            paste && lastCopied && lastCopied.lineWise &&
    ...
    function addEditorMethods(CodeMirror) {
      ...
      CodeMirror.prototype = {
        ...
        lineAtHeight: function (height, mode) {
          height =
            fromCoordSystem(this, { top: height, left: 0 }, mode || "page").top;
        ...
        setSize: methodOp(function (width, height) {
          ...
          if (width != null) {
            this.display.wrapper.style.width = interpret(
              width,
            );
          }
          if (height != null) {
        ...
        refresh: methodOp(function () {
          ...
          if (
            oldHeight == null ||
            Math.abs(oldHeight - textHeight(this.display)) > .5 ||
    ...
    var ContentEditableInput = function (cm) {
      this.cm = cm;
      this.lastAnchorNode = this.lastAnchorOffset = this.lastFocusNode = this
    ...
    ContentEditableInput.prototype.showPrimarySelection = function () {
      ...
      if (
        ...
        from.line >= cm.display.viewTo || to.line < cm.display.viewFrom
      ) {
        sel.removeAllRanges();
        return;
      }
    ...
    ContentEditableInput.prototype.pollSelection = function () {
      if (
        this.readDOMTimeout != null || this.gracePeriod ||
      ...
      if (
        android && chrome && this.cm.display.gutterSpecs.length &&
    ...
    ContentEditableInput.prototype.pollContent = function () {
      ...
      if (
        to.ch == getLine(cm.doc, to.line).text.length && to.line < cm.lastLine()
      ) {
        to = Pos(to.line + 1, 0);
      }
      if (
        from.line < display.viewFrom || to.line > display.viewTo - 1
      ) {
        return false;
      }
    ...
    function domTextBetween(cm, from, to, fromLine, toLine) {
      ...
      function walk(node) {
        if (node.nodeType == 1) {
          ...
          if (
            !/^br$/i.test(node.nodeName) && node.textContent.length == 0
          ) {
            return;
          }
    ...
    function domToPos(cm, node, offset) {
      ...
      if (node == cm.display.lineDiv) {
        ...
        for (lineNode = node;; lineNode = lineNode.parentNode) {
          ...
          if (
            lineNode.parentNode && lineNode.parentNode == cm.display.lineDiv
          ) {
            break;
          }
        }
    ...
    function locateNodeInLineView(lineView, node, offset) {
      ...
      if (
        !textNode && node.childNodes.length == 1 &&
    ...
    TextareaInput.prototype.poll = function () {
      ...
      if (
        this.contextMenuPending || !cm.state.focused ||
      ...
      while (
        same < l && prevInput.charCodeAt(same) == text.charCodeAt(same)
      ) {
        ++same;
      }
