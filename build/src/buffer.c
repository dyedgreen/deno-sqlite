...
buffer* get_reg_buffer(int id) {
  if (!VALID_REG_ID(id))
    return NULL;
  // Ensure buffer_registry exists
  if (buffer_registry == NULL) {
    buffer_registry = malloc(sizeof(buffer*) * BUFFER_REG_SIZE);
    if (buffer_registry == NULL)
      return NULL;
    for (int i = 0; i < BUFFER_REG_SIZE; i ++)
      buffer_registry[i] = NULL;
  }
  // Ensure buffer at id exists
  if (buffer_registry[id] == NULL)
    buffer_registry[id] = new_buffer();
  return buffer_registry[id];
}
...
int in_use_reg_buffer_id(int id) {
  ...
  if (buffer_registry == NULL)
    return false;
  return buffer_registry[id] != NULL;
}
...
void delete_reg_buffer(int id) {
  ...
  if (buffer_registry == NULL)
    return;
  destroy_buffer(buffer_registry[id]);
  buffer_registry[id] = NULL;
}
